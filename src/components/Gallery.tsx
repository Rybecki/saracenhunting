...
import { ArrowRight, ChevronLeft, ChevronRight, Maximize2, X, ChevronDown } from "lucide-react";
import { GALLERY_ITEMS, HOME_GALLERY_PREVIEW_IDS } from "../data";
...
function getTeaserItems() {
  return HOME_GALLERY_PREVIEW_IDS.map((id) => GALLERY_ITEMS.find((item) => item.id === id)).filter(
    (item): item is (typeof GALLERY_ITEMS)[number] => item !== undefined
  );
}
...
export default function Gallery({
  ...
  const teaserItems = getTeaserItems();
  ...
  const renderGalleryItem = (item: (typeof GALLERY_ITEMS)[number], masonry = false) => (
    <motion.div
      layout
      ...
      className={`relative overflow-hidden rounded-lg border border-brand-forest/30 group cursor-pointer ${
    ...
    >
      <img
        ...
        className={`w-full object-cover rounded-lg filter brightness-90 group-hover:brightness-100 group-hover:scale-[1.03] transition-all duration-500 ${
          masonry ? "h-auto" : "aspect-[4/3]"
        }`}
        loading="lazy"
      />
        ...
        {isTeaser ? (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {teaserItems.map((item) => renderGalleryItem(item))}
            </div>
          ...
          <>
            <div className="sm:hidden">
              <motion.div layout className="space-y-0">
                <AnimatePresence mode="popLayout">
                  {mobileItems.map((item) => renderGalleryItem(item, true))}
                </AnimatePresence>
              </motion.div>
            ...
            <motion.div layout className="hidden sm:block columns-2 md:columns-3 gap-4 space-y-4">
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item) => renderGalleryItem(item, true))}
              </AnimatePresence>
            </motion.div>
          </>
        )}
